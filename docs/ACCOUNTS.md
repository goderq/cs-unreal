...
- Страница **PROFILE** в меню (или щелчок по карточке с ником): матчи, победы,
  убийства, смерти, K/D, хедшоты и время в игре, смена ника и таблица лидеров
  по убийствам. Ник уникален на всех игроков, длина 3..20 — это проверяет
  база, а не только игра.
...
- Проверить вход и смену ника по-настоящему (с обращением к Epic и базе):

  ```
  UnrealEditor.exe CSFusion.uproject -game -cstestlogin
  UnrealEditor.exe CSFusion.uproject -game -cstestmenu=rename:НовыйНик
  ```

  Первый пишет в лог `LOGIN TEST BACKEND` (поднялся ли EOS) и состояние
  аккаунта, второй — `RENAME TEST RESULT`. В общий набор тестов смена ника не
  входит: она меняет данные в настоящей базе.
