...
2. Выложи три функции из `Backend/supabase/functions/`. Проще всего прямо в
   дашборде: **Edge Functions → Deploy a new function → Via Editor**, вставить
   код, задать имя (`eos-login`, `report-match`, `admin`) и нажать Deploy.
...
- Страница **PROFILE** в меню (или щелчок по карточке с ником): матчи, победы,
  убийства, смерти, K/D, хедшоты и время в игре и таблица лидеров по
  убийствам.
- Ник берётся из Epic при первом входе (если он занят — с приставкой вида
  `#1234`) и с v2.0 игроком **не меняется**: у игроков нет права записи в
  таблицы, а триггер `profiles_guard` дополнительно откатывает ник, бан и
  админку, если их попробует поменять роль игрока. Переименовать может только
  админ (см. §5). Ник уникален, длина 3..20 — это проверяет база.
...
В меню есть страница **ADMIN**. Её видят только аккаунты с `is_admin = true`
в таблице `profiles`, но прятать кнопку — не защита: каждое действие идёт
через функцию `admin`, а она при **каждом** вызове проверяет по базе, что
вызывающий — админ. Снятая админка действует сразу, даже с открытой панелью.

Что умеет панель:

- поиск игроков по нику, список с последним входом и статистикой;
- бан на 1 час, 1 день, 7 дней или навсегда и снятие бана (забаненный не
  проходит `eos-login`);
- выдать или забрать админку (себя разжаловать нельзя — чтобы не остаться без
  админов);
- переименовать игрока (3..20 символов, занятый ник → ошибка);
- обнулить статистику (кнопку нужно нажать дважды);
- журнал: кто, что и с кем сделал (таблица `admin_log`, пишет только функция).

**Первого админа** назначают в дашборде: **SQL Editor** →

```sql
update public.profiles set is_admin = true where nickname = 'ТвойНик';
```

Дашборд работает под ролью `postgres`, и `profiles_guard` его не
ограничивает. Дальше админов можно назначать прямо из панели. Флаг приходит в
игру при входе, поэтому после назначения нужно перезапустить игру.
...
- Проверить вход по-настоящему (с обращением к Epic и базе):

  ```
  UnrealEditor.exe CSFusion.uproject -game -cstestlogin
  ```

  В лог пишется `LOGIN TEST BACKEND` (поднялся ли EOS) и состояние аккаунта.
