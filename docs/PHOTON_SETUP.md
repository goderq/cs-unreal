...
Регион выбирается на странице PLAY в меню, флагом `-region=CODE` или
задаётся полем запроса
[`FCSSessionRequest`](../Source/CSFusion/Multiplayer/CSSessionSubsystem.h):

| Выбор | Регион подключения |
|---|---|
| ничего не выбрано («Default (Europe)» в меню, без `-region`) | `DefaultRegion` из `Config/DefaultGame.ini`, сейчас `eu` |
| `bSelectRegion = true`, `Region = "us"`, `"ru"`, … | этот регион (`EFusionRegionSelectionMode::Select`) |

**Режим «лучший пинг» (`Best`) с v1.0 не используется.** В тестах два
клиента на одном компьютере выбрали разные «лучшие» регионы (asia и eu).
Имя комнаты уникально только внутри региона, поэтому друзья, вошедшие в
«одну и ту же» комнату, оказывались в двух разных. Поменять общий регион
можно в `DefaultGame.ini`:

```ini
[/Script/CSFusion.CSSessionSubsystem]
DefaultRegion=eu
```

Пустое значение возвращает режим `Best`. Делать так не рекомендуется.
...
| `-region=CODE` | жёсткий регион (`eu`, `us`, …) | `DefaultRegion` из конфига (`eu`) |
